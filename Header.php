...
//});
   ...
   j('li#vwCus').click(function(e) 
	...
	j('li#regItm').click(function(e) 
	   { 
		j( "#container" ).load( "Add_Scheme.php" );
	   });
	   
    j('li#vwItm').click(function(e) 
	   { 
		j( "#container" ).load( "View_Schemes.php" );
	   });
	...
	j('li#MchnItms').click(function(e) 
	   { 
		...
		j( "#container" ).load( "Create_Loan.php" );
	 ...
	 j('li#Contr').click(function(e) 
	   { 
		...
		qry='SELECT * FROM loan_header';

		qry = qry.replace(/\s+/g, '-');
		
		
		j( "#container" ).load( "Loans.php?qry="+qry);
			clearInterval(myVar);
			myVar = setInterval(paidClrChnger, 1000);
			
	   });
	   
	   j('#container').unload(function(){
...
//=================================================================================================================
		if((LoadStaus1=='')&&(tabId=='HireTab1'))
		{
		  ...
		  j( "#HireCustomer" ).load( "phpFunctions/getLoanCustomers.php?qry="+qry );
		  //alert('tab 1 loaded');
		  LoadStaus1='load';
		 }
		 else if((LoadStaus2=='')&&(tabId=='HireTab2'))
		{
		  j( "#HireItems" ).load( "phpFunctions/getLoanSchemes.php" );
		  //alert('tab 2 loaded');
		  LoadStaus2='load';
		 }
 ...
 var ClCode = selectedCusCode;
	...
	if(selectedCusCode!='')
	{
		  if(lastChecked!='-1')
		  {
					...
					{
						type: 'post',
						url: 'phpFunctions/SaveLoan.php',
						data: { CliCode:ClCode,
								Lnamt:LoanAmt,
								Lndays:LoanDays,
								LnChrgPerDay:ChgPerDay,
								LnschmID:schemaID
							   },
						success: function(result)
						{
									 j('input#ContrNo').val(result);
									 //j('#tempDate').html(result);
...
{
 //alert("close");
 j("#closedBanner").css("display", "block");
}
</script>
...
function SrchContrByContrNo(event)
{
	...
	{
 		qry='SELECT * FROM loan_header WHERE LOAN_ID="'+id+'" ';
		qry = qry.replace(/\s+/g, '-');
	    j( "#container" ).load( "Loans.php?qry="+qry);
	}
...
function SrchContrByCusId(event)
{
	...
	{
 		qry='SELECT * FROM loan_header cc , customer c where cc.CL_CODE=c.CL_CODE AND  c.CL_NIC="'+nic+'" ';
		qry = qry.replace(/\s+/g, '-');
		j( "#container" ).load( "Loans.php?qry="+qry);
	}
...
function paidClrChnger() 
{
    ...
    Bgcolor=j("#LoanDetBox table#schdlTbl tr td.blink").css("background-color");
	
	
	if(Bgcolor=='rgb(214, 214, 214)')
	{
		j("#LoanDetBox table#schdlTbl tr td.blink").css("background-color", "rgb(249, 4, 4)");
	}
	else if(Bgcolor=='rgb(249, 4, 4)')
	{
		j("#LoanDetBox table#schdlTbl tr td.blink").css("background-color", "rgb(214, 214, 214)");
	}
...
function Selectedscheme(scheme,days,obj,perDay,schmId)
{
//
	if (obj.checked)
    {
		...
		LoanDays=days;
		ChgPerDay=perDay;
		j("#"+lastChecked).attr("checked",false);
		lastChecked=obj.id;
		schemaID=schmId;
        //alert(scheme+' '+lastChecked);
    }
    else
    {
...
{
	var heightWindow=0;
	//heightWindow=j(window).height()+"px"; 

	heightWindow=(window.innerHeight-10);

	j("#MenuBar").css("height",heightWindow+"px");
	j("html").css("height",heightWindow+"px");
	j("#HireTabContainer").css("height",heightWindow+"px");
...
<script>
function adminShowPayBox(LnID,date,amt,tdID,arrAmt)
{
//j( "p" ).removeClass( "myClass noClass" ).addClass( "yourClass" );
document.getElementById("UpdateDepositBox").style.display = "block";


j("#DlyChrg").val(amt);
j("#ArrAmt").val(arrAmt);
j("#LnIDholder").val(LnID);
j("#LnDateholder").val(date);

	j( "#LoanDetBox table#schdlTbl tr td#"+tdID ).removeClass( "blink" );
	//j( "#LoanDetBox table#schdlTbl tr td#"+tdID ).css("background-color", "rgb(6, 219, 88)");
}

function adminSavePay(LID)
{

var payingAmt = j('#PyingAmt').val();
var DlyChrg = j('#DlyChrg').val();
var ArrAmt = j('#ArrAmt').val();
var lnid = j("#LnIDholder").val();
var LnDateholder = j("#LnDateholder").val();



var  shouldPay = 0;
 	 shouldPay = (parseFloat(ArrAmt)-parseFloat(payingAmt));


	if(payingAmt=='' || payingAmt==0)
	{
		alert('Please Enter the Paying Amount');
	}
	else if(payingAmt>ArrAmt)
	{
		alert('Paying Amount Exceeded');
	}
	else
	{
		j(".preloader").css("display", "block");
				...
				{
				type: 'post',
				url: 'phpFunctions/SaveDailyCollection.php',
				data: {
						pyingAmt: payingAmt,
						LnDtholder:LnDateholder,
						loanid:lnid
					   },
				success: function(result)
						{
							 j( "#container" ).load( "phpFunctions/ViewContractDet.php?Contr_No="+LID );
							 j('.preloader').fadeOut('slow',function(){ j(".preloader").css("display", "none");});
							 //j('#temp').append(result);
							 //alert(result);
							 j( "#UpdateDepositBox").slideUp( "slow", function() {});
